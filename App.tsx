...
const CompositeThumbnail: React.FC<{ file: File; isSelected: boolean; onClick: () => void; }> = ({ file, isSelected, onClick }) => {
    ...
    return (
        <button onClick={onClick} className={`relative aspect-square w-full rounded-lg overflow-hidden transition-all duration-200 border-4 ${isSelected ? 'border-blue-500 scale-105 shadow-2xl' : 'border-transparent hover:border-gray-500'}`}>
...
const BatchThumbnail: React.FC<{ file: File; }> = ({ file }) => {
    ...
    return (
        <div className="relative aspect-square w-full rounded-lg overflow-hidden border-2 border-gray-700">
...
const App: React.FC = () => {
  ...
  const handleUndo = useCallback(() => { if (canUndo) { setHistoryIndex(i => i - 1); setEditHotspot(null); setDisplayHotspot(null); } }, [canUndo]);
  const handleRedo = useCallback(() => { if (canRedo) { setHistoryIndex(i => i + 1); setEditHotspot(null); setDisplayHotspot(null); } }, [canRedo]);
  const handleReset = useCallback(() => { if (history.length > 0) { setHistoryIndex(0); setError(null); setEditHotspot(null); setDisplayHotspot(null); setSelectedIndices(new Set()); } }, [history]);
  const handleUploadNew = useCallback(() => { setHistory([]); setHistoryIndex(-1); setError(null); setPrompt(''); setEditHotspot(null); setDisplayHotspot(null); setCompositeFiles([]); setStyleSourceIndex(0); setBatchFiles([]); setActiveTool('retouch'); setSelectedIndices(new Set()); }, []);
      ...
      switch (activeTool) {
          case 'composite':
              return <div className="flex flex-col items-center gap-4 animate-fade-in">
                  <h3 className="text-xl font-bold text-gray-200 self-start">Image Compositing</h3>
                  <p className="text-md text-gray-400">
                      Select an image from the main view to be the <span className="font-bold text-blue-400">Style Source</span>. Its lighting and mood will be applied to the other images. Then, click Generate.
                  </p>
                  <p className="text-sm text-gray-400 bg-gray-900/50 p-3 rounded-md border border-gray-700 w-full text-center">
                  ...
                  <button onClick={() => { setCompositeFiles([]); setStyleSourceIndex(0); }} className="text-center bg-transparent border border-white/20 text-gray-200 font-semibold py-3 px-4 rounded-md transition-all duration-200 ease-in-out hover:bg-white/10 w-full mt-4">
                      Start Over
                  </button>
              </div>;
          case 'retouch':
              return <div className="flex flex-col items-center gap-4 animate-fade-in">
                  <h3 className="text-xl font-bold text-gray-200 self-start">Precise Retouching</h3>
                  ...
                  <form onSubmit={(e) => { e.preventDefault(); handleGenerate(); }} className="w-full flex flex-col items-center gap-2">
                      <input type="text" value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder={editHotspot ? "e.g., 'change shirt to blue'" : "First click the image"} className="bg-gray-800 border border-gray-700 text-gray-200 rounded-lg p-4 text-base focus:ring-2 focus:ring-blue-500 focus:outline-none transition w-full disabled:cursor-not-allowed disabled:opacity-60" disabled={isLoading || !editHotspot} />
          ...
          case 'replace':
              return <div className="flex flex-col items-center gap-4 animate-fade-in">
                  <h3 className="text-xl font-bold text-gray-200 self-start">Replace Object</h3>
                  <p className="text-md text-gray-400">Describe the object to replace and what to replace it with.</p>
                  <form onSubmit={(e) => { e.preventDefault(); handleReplaceObject(); }} className="w-full flex flex-col items-center gap-2">
                      <input type="text" value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="Object to replace (e.g., 'the blue car')" className="bg-gray-800 border border-gray-700 text-gray-200 rounded-lg p-4 text-base focus:ring-2 focus:ring-blue-500 focus:outline-none transition w-full" disabled={isLoading} />
                      <input type="text" value={secondaryPrompt} onChange={(e) => setSecondaryPrompt(e.target.value)} placeholder="Replace with (e.g., 'a red bicycle')" className="bg-gray-800 border border-gray-700 text-gray-200 rounded-lg p-4 text-base focus:ring-2 focus:ring-blue-500 focus:outline-none transition w-full" disabled={isLoading} />
  ...
  const renderContent = () => {
    ...
    if (activeTool === 'composite' && compositeFiles.length === 0) {
        return <div className="w-full h-full flex items-center justify-center p-8">
            <div className="text-center max-w-2xl animate-fade-in flex flex-col items-center gap-6">
                <CompositeIcon className="w-16 h-16 text-blue-400" />
                <h2 className="text-4xl font-bold text-gray-100">Image Composite Tool</h2>
                <p className="text-lg text-gray-400">Select two or more images to blend together. One image will provide the style, and the others will provide the content.</p>
    ...
    if (activeTool === 'batch' && batchFiles.length === 0) {
        return <div className="w-full h-full flex items-center justify-center p-8">
            <div className="text-center max-w-2xl animate-fade-in flex flex-col items-center gap-6">
                <BatchIcon className="w-16 h-16 text-blue-400" />
                <h2 className="text-4xl font-bold text-gray-100">Batch Processing Tool</h2>
                <p className="text-lg text-gray-400">Select multiple images to apply the same effect to all of them at once. Pin a frame in the 'Frame' tool to get started.</p>
    ...
    return (
      <div className="w-full h-full flex animate-fade-in">
        ...
        <div className="flex-grow flex flex-col items-center justify-start p-4 md:p-8 gap-4 overflow-y-auto">
            {isLoading && <div className="absolute inset-0 bg-black/70 z-30 flex flex-col items-center justify-center gap-4 animate-fade-in"><Spinner /><p className="text-gray-300">AI is working its magic...</p></div>}
            
            {activeTool === 'composite' && compositeFiles.length > 0 ? (
              <div className="w-full max-w-5xl">
                  <h2 className="text-2xl font-bold text-gray-200 mb-4 px-2">Composite Sources</h2>
              ...
              <div className="w-full max-w-5xl">
                  <h2 className="text-2xl font-bold text-gray-200 mb-4 px-2">Batch Images ({batchFiles.length})</h2>
                  <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                ...
                <>
                  <div className="relative w-full max-w-5xl shadow-2xl rounded-xl overflow-hidden bg-black/20 flex items-center justify-center">
                      ...
                      {displayHotspot && !isLoading && activeTool === 'retouch' && <div className="absolute rounded-full w-6 h-6 bg-blue-500/50 border-2 border-white pointer-events-none -translate-x-1/2 -translate-y-1/2 z-10" style={{ left: `${displayHotspot.x}px`, top: `${displayHotspot.y}px` }}><div className="absolute inset-0 rounded-full w-6 h-6 animate-ping bg-blue-400"></div></div>}
                  </div>
                  {history.length > 1 && <div className="w-full max-w-5xl">
                      <h3 className="text-sm font-bold text-gray-400 mb-2 px-2">History</h3>
                      <div className="flex gap-2 overflow-x-auto p-2 bg-black/20 rounded-lg">
                        {history.map((imgFile, index) => <ImageThumbnail key={`${imgFile.lastModified}-${index}`} file={imgFile} isSelected={index === historyIndex} onClick={() => { setHistoryIndex(index); setEditHotspot(null); setDisplayHotspot(null); }} isSelectedForDownload={selectedIndices.has(index)} onSelectToggle={() => handleToggleSelection(index)} />)}
                      </div>
                  </div>}
                </>
            )}
        ...
        <aside className="w-[380px] flex-shrink-0 bg-gray-800/30 border-l border-gray-700/80 p-6 flex flex-col gap-6 backdrop-blur-sm overflow-y-auto">
            {renderControlPanel()}
            
            {/* Common controls are only shown in editing mode, not composite or batch mode */}
            {currentImage && 
              <div className="mt-auto pt-6 border-t border-gray-700 flex flex-col gap-4">
                <div className="grid grid-cols-2 gap-2">
                  <button onClick={handleUndo} disabled={!canUndo || isLoading} className="flex items-center justify-center text-center bg-white/10 border border-white/20 text-gray-200 font-semibold py-3 px-4 rounded-md transition-all duration-200 ease-in-out hover:bg-white/20 disabled:opacity-50 disabled:cursor-not-allowed" aria-label="Undo"><UndoIcon className="w-5 h-5 mr-2" />Undo</button>
                  <button onClick={handleRedo} disabled={!canRedo || isLoading} className="flex items-center justify-center text-center bg-white/10 border border-white/20 text-gray-200 font-semibold py-3 px-4 rounded-md transition-all duration-200 ease-in-out hover:bg-white/20 disabled:opacity-50 disabled:cursor-not-allowed" aria-label="Redo"><RedoIcon className="w-5 h-5 mr-2" />Redo</button>
                  {canUndo && <button onMouseDown={() => setIsComparing(true)} onMouseUp={() => setIsComparing(false)} onMouseLeave={() => setIsComparing(false)} onTouchStart={() => setIsComparing(true)} onTouchEnd={() => setIsComparing(false)} disabled={isLoading} className="col-span-2 flex items-center justify-center text-center bg-white/10 border border-white/20 text-gray-200 font-semibold py-3 px-4 rounded-md transition-all duration-200 ease-in-out hover:bg-white/20 disabled:opacity-50" aria-label="Compare"><EyeIcon className="w-5 h-5 mr-2" />Compare with Original</button>}
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <button onClick={handleReset} disabled={!canUndo || isLoading} className="text-center bg-transparent border border-white/20 text-gray-200 font-semibold py-3 px-4 rounded-md transition-all duration-200 ease-in-out hover:bg-white/10 disabled:opacity-50 disabled:cursor-not-allowed">Reset</button>
                  <button onClick={handleUploadNew} disabled={isLoading} className="text-center bg-white/10 border border-white/20 text-gray-200 font-semibold py-3 px-4 rounded-md transition-all duration-200 ease-in-out hover:bg-white/20 disabled:opacity-50">Upload New</button>
                </div>

                <div className="flex flex-col gap-3 pt-4 border-t border-gray-700">
                    <h3 className="text-md font-semibold text-gray-300 flex items-center gap-2"><DownloadIcon className="w-5 h-5" />Downloads</h3>
  ...
  return (
    <div className="min-h-screen text-gray-100 flex flex-col overflow-hidden">
      <Header />
