...
const Header: React.FC<HeaderProps> = ({ theme, setTheme }) => {
  ...
  return (
    <header className="w-full py-4 px-8 border-b border-slate-200 dark:border-gray-700 bg-white/70 dark:bg-gray-800/30 backdrop-blur-sm sticky top-0 z-50 flex items-center justify-between">
      <div className="flex-1"></div>
      <div className="flex flex-1 items-center justify-center gap-3">
          <SparkleIcon className="w-8 h-8 text-blue-500 dark:text-blue-400" />
          <h1 className="text-3xl font-bold tracking-tight text-slate-800 dark:text-gray-100">
            DreamCraft
          </h1>
      </div>
      <div className="flex-1 flex justify-end">
          <button 
              onClick={toggleTheme}
              className="p-2 rounded-full transition-colors duration-200 text-slate-600 dark:text-gray-400 hover:bg-slate-200 dark:hover:bg-gray-700"
              aria-label={`Switch to ${theme === 'light' ? 'dark' : 'light'} mode`}
          >
              {theme === 'light' ? <ThemeMoonIcon className="w-6 h-6" /> : <ThemeSunIcon className="w-6 h-6" />}
          </button>
      </div>
    </header>
  );
